...
//    ======================================================================================================================
    ...
    public function add($id_edit=null)
    {
        ...
        if (isset($_POST['addnews'])) {
            ...
            $this->thumImage($image, $pathImage, $dir_image, $id);
            ...
            if ($cat->resource == 'attribute') {
                ...
                foreach ($data['attribute']->cat_sub as $key2 => $cat2) {
                    if ($cat2->resource == 'listBrand') {
                        ...
                        $data['cat_brand'] = $this->productmodel->get_data('product_hangsx',array(
                            'lang' => $this->language
                        ),null);
                    }
    ...
    private function tagsPro ($arrTags, $id) {
        if ($arrTags > 0) {
    ...
    private function thumImage ($image, $pathImage, $dir_image, $id) {
        if($image !='') {
            #BEGIN: Create thumbnail tạo anh thumb chen chữ
            $wm_font_size = 28;
        ...
        if (@$image == 'none.gif') {
            ...
            if (trim($dir_image) != '' &&
                trim($dir_image) != 'default' &&
                is_dir('upload/img/product/'.$dir_image) &&
                count($this->file->load('upload/img/product/'.$dir_image,'index.html')) == 0) {
