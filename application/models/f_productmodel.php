...
public function count_ProbyCate($id)
    ...
    public function getHangsx($where)
    {
        $query = $this->db->select('
            product.id,
            product.name as pro_name,
            product.price_sale,
            product_hangsx.id as hang_id,
            product_hangsx.alias as cate_alias,
            product_hangsx.name
        ')
            ->from('product')
            ->join('product_hangsx', 'product_hangsx.id=product.hangsx_id')
            ->join('product_to_category', 'product.id=product_to_category.id_product','left')
            ->join('product_category', 'product_category.id=product_to_category.id_category','left')
            ->where($where)
            ->order_by('product_hangsx.id', 'desc')
            ->group_by('product_hangsx.id')
            ->get();
    ...
    public function tinhsao($id){
        $this->db->select('
            SUM(comments_binhluan.giatri) as tongdanhgia,
            COUNT(comments_binhluan.id_sanpham) as soluong
        ');
        $this->db->where('comments_binhluan.id_sanpham', $id);
        $this->db->group_by('comments_binhluan.id_sanpham');
        $q=$this->db->get('comments_binhluan');
        return $q->first_row();
    }

    public function getwherejoinsize($id){
        $this->db->select('product_to_size.*, product_size.name');
        $this->db->from('product_to_size');
        $this->db->join('product_size', 'product_size.id = product_to_size.id_size');
        $this->db->where('product_to_size.id_product', $id);
        $data = $this->db->get();
...
// sô binh luan cua sp
    ...
    public function getProductSimilar($product_id,$catid, $limit, $offset)
    {
        ...
        $query = $this->db->select('product.id as pro_id,product_category.id,product.alias,product.image,product.caption_1, product.time,
