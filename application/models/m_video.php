...
class M_video extends MY_Model{
    ...
    public function getVideoByCategory($id,$limit,$offset){
        $query = $this->db->select('video.*, video_category.name as name_cate, video_category.alias as alias_cate ')
            ->from('video')
            ->join('video_category', 'video_category.id = video.category_id')
