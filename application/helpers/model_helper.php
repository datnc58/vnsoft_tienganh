...
//view danh muc dang check list table_to_table
function view_product_cate_checklist1($data,$parent=0,$text='',$edit=null){
	foreach ($data as $k=>$v) {
        if ($v->parent_id == $parent) {
            unset($data[$k]);
            $id = $v->id;
            ...
            echo '<div class="checkbox">
                        <label>
                          '.$text.'<input type="radio" style="width: 20px; height: 20px;" name ="category[]" value="'.$v->id.'"'.@$selected.'
                          class="chk" id="'.$v->id.'">
                          '.$v->name.'
                                </label>
                      </div> ';

            view_product_cate_checklist1($data, $id, $text . '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',$edit);
            echo "</ul><li>";
        }
    }
}
...
function check_hassubs1($id_root,$arr_sub){
    $check = false;
